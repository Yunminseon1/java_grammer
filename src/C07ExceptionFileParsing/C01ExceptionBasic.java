...
public class C01ExceptionBasic {
    static void main(String[] args) {
       ...
       // 예외처리 목적 : 1) 사용자에게 적절한 에러 메시지 전달  2) 예외발생시 디버깅을  위한 로그기록
        ...
        }catch (ArithmeticException error){  // catch를 통해 예상되는 예외클래스를 분기처리
            System.out.println("0으로 나누시면 안됩니다.");  // 사용자에게 메시지 전달(가정)
            error.printStackTrace();  // 시스템에 디버깅을 위한 로그 자세히 기록

        } catch (NumberFormatException e){
            System.out.println("문자를 입력하시면 안됩니다.");
            e.printStackTrace();
          
        }catch (Exception e){  // Exception은 모든 예외클래스의 조상클래스. 구체적인 예외클래스가 catch부에 있다면, 해당부분으로 우선분기처리.
        ...
        System.out.println("예외발생하더라도 예외처리를 한 후에 발생하는 코드");  // 예외처리를 하지 않을 경우 프로그램은 강제 종료. 예외처리를 할경우 프로그램은 예외가 발생하더라도 지속.
    }
